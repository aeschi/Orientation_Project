...
d3.csv('data/skate_boulder_swim_labeled.csv', function (data) {
    ...
    function createSphereNoise() {
        ...
        // sphereMaterial = new THREE.MeshLambertMaterial({ color: '#FFB742' });
        ...
        for (let i = 1; i < gravity.length; i += 12) {
            let scaling = 1.3;
    ...
    function buildGui() {
        ...
        dataInput.add(params, 'dataSource', ['bouldering', 'skating', 'swimming']).onChange(setDataInput);

        dataInput.open();

        let dataSrc = gui.addFolder('Data Sources');
        dataSrc.add(params, 'gravity').onChange(changePar);
